...
//////////////////////////////////////////////////////////////////////
CString CMatrix::ToString(const CString& sDelim, BOOL bLineBreak, const CString& sFormat) const
{
	CString s=_T("");
	...
	for (int i=0; i<m_nNumRows; ++i)
	{
		...
		{
			CString ss;
			ss.Format(sFormat, GetElement(i, j));
			s += ss;
...
void CMatrix::ToString(CString& outS, const CString& sDelim, BOOL bLineBreak, const CString& sFormat) const
{
	for (int i=0; i<m_nNumRows; ++i)
	{
		for (int j=0; j<m_nNumColumns; ++j)
...
void CMatrix::ToFile(const CString& sPahtName) const
{
	CString s = _T("");
	ToString(s, _T(" "), TRUE, _T("%10.4f"));
	CFile file;
	if(!file.Open(sPahtName, CFile::modeCreate|CFile::modeWrite))
...
BOOL CMatrix::SetElements(int nStartRow, int nStartCol, int nRow, int nCol, CMatrix value)
{
	...
	for(int i = 0; i < nRow; i ++)
		...
		{
			m_pData[nStartCol + j + (nStartRow + i) * m_nNumColumns] =
...
CMatrix CMatrix::GetRowVector(int nRow) const
{
	// 构造目标矩阵
	CMatrix	row(1, m_nNumColumns);
	for (int j=0; j<m_nNumColumns; ++j)
		row.SetElement(0,j,GetElement(nRow, j));

	return row;
}

void CMatrix::SetRowVector(int nRow, const CMatrix& other)
{
	ASSERT(other.GetNumRows() == 1 && m_nNumColumns == other.GetNumColumns());
	for (int j=0; j<m_nNumColumns; ++j)
		SetElement(nRow,j,other.GetElement(0,j));
}

void CMatrix::SetColVector(int nCol, const CMatrix& other)
{
	ASSERT(other.GetNumColumns() == 1 && m_nNumRows == other.GetNumRows());
	for (int j=0; j<m_nNumRows; ++j)
		SetElement(j, nCol,other.GetElement(j,0));
}
...
//
// 参数：
// 1. const CMatrix& other - 与自己相加的矩阵
//////////////////////////////////////////////////////////////////////
void CMatrix::Add(const CMatrix& other)
{
	// 首先检查行列数是否相等
	ASSERT (m_nNumColumns == other.GetNumColumns() && m_nNumRows == other.GetNumRows());
...
//
// 参数：
// 1. const CMatrix& other - 被减的矩阵
//
//////////////////////////////////////////////////////////////////////
void CMatrix::Sub(const CMatrix& other)
{
	// 首先检查行列数是否相等
	ASSERT (m_nNumColumns == other.GetNumColumns() && m_nNumRows == other.GetNumRows());
...
//
// 参数：
// 1. double value - 与自己相乘的实数
//
//////////////////////////////////////////////////////////////////////
void CMatrix::Mul(double value)
{
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		for (int j = 0 ; j <  m_nNumColumns; ++j)
...
//
// 参数：
// 1. double value - 被自己相除的实数
//////////////////////////////////////////////////////////////////////
void CMatrix::Div(double value)
{
	ASSERT(value != 0.0);
...
//
// 参数：
// 1. const CMatrix& other - 与指定矩阵进行点乘的矩阵
//
// 返回值：CMatrix型，指定矩阵与other点乘结果
//////////////////////////////////////////////////////////////////////
CMatrix	CMatrix::operator^(const CMatrix& other) const
{
	// 首先检查行列数是否相等
...
void CMatrix::Transpose(CMatrix* trans) const
{
	ASSERT (m_nNumColumns == trans->GetNumRows() && m_nNumRows == trans->GetNumColumns());
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		for (int j = 0 ; j < m_nNumColumns ; ++j)
			trans->SetElement(j, i, GetElement(i, j)) ;
...
double CMatrix::SquaredDistance()
{
	double	value = 0.0;
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
...
BOOL CMatrix::Copy(int nRowTarget, int nColTarget, int nRowBegin, int nColBegin, int nRowEnd, int nColEnd, const CMatrix *value)
{
	int i, j, h, w, h_done, w_done;
	
	if ((nRowTarget >= 0)&&(nRowTarget < m_nNumRows)&&(nColTarget >= 0)&&(nColTarget < m_nNumColumns)) 
	{
		if ((nRowBegin >= 0)&&(nRowBegin < value->GetNumRows())&&(nColBegin >= 0)&&(nColBegin < value->GetNumColumns())) 
		{
			h = nRowEnd-nRowBegin+1;
			w = nColEnd-nColBegin+1;
			if ((h >= 1)&&(w >= 1)) 
			{
				h_done = nRowTarget+h-1;
				w_done = nColTarget+w-1;
				if ((h_done < m_nNumRows)&&(w_done < m_nNumColumns)) 
				{
					for (i=0;i<h;i++) 
					{
						for (j=0;j<w;j++) 
						{
							m_pData[nColTarget + j + (nRowTarget + i) * m_nNumColumns] = value->GetElement(i+nRowBegin, j+nColBegin);
						}
					}
					return TRUE;
				}
...
void CMatrix::centdiffX(CMatrix& dx) const
{
	// 首先检查行列数是否相等
	ASSERT (m_nNumColumns == dx.GetNumColumns() && m_nNumRows == dx.GetNumRows());

	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		dx.SetElement(i, 0, GetElement(i,1) - GetElement(i,0));
		for (int j = 1 ; j < m_nNumColumns - 1 ; ++j)
		{
			dx.SetElement(i, j, 0.5 * (GetElement(i,j+1) - GetElement(i,j-1)));
		}
		dx.SetElement(i, m_nNumColumns - 1, GetElement(i,m_nNumColumns - 1) - GetElement(i,m_nNumColumns - 2));
	}
}

void CMatrix::centdiffY(CMatrix& dy) const
{
	// 首先检查行列数是否相等
	ASSERT (m_nNumColumns == dy.GetNumColumns() && m_nNumRows == dy.GetNumRows());

	for (int j = 0 ; j < m_nNumColumns ; ++j)
	{
		dy.SetElement(0, j, GetElement(1,j) - GetElement(0,j));
		for (int i = 1 ; i < m_nNumRows - 1 ; ++i)
		{
			dy.SetElement(i, j, 0.5 * (GetElement(i+1,j) - GetElement(i-1,j)));
		}
		dy.SetElement(m_nNumRows - 1, j, GetElement(m_nNumRows - 1,j) - GetElement(m_nNumRows - 2,j));
	}
}
...
%
*/
void CMatrix::thomas(const CMatrix& a, const CMatrix& b, const CMatrix& c, const CMatrix& d)
{
	ASSERT (m_nNumColumns == a.GetNumColumns() && m_nNumColumns == b.GetNumColumns() && m_nNumColumns == c.GetNumColumns() && m_nNumColumns == d.GetNumColumns());
	ASSERT (m_nNumRows == a.GetNumRows() && m_nNumRows == (b.GetNumRows() + 1) && m_nNumRows == (c.GetNumRows() + 1) && m_nNumRows == d.GetNumRows());

	// Initialization
	CMatrix m(a.GetNumRows(),a.GetNumColumns());
	CMatrix l(c.GetNumRows(),c.GetNumColumns());
	CMatrix y(d.GetNumRows(),d.GetNumColumns());
	int n = a.GetNumRows();
...
/*	int _x, _y;
	for (_y = 0; _y < m_nNumColumns; _y++)
	{
		//2. Forward substitution (L*y=d, for y) 
		m.SetElement(0, _y, a.GetElement(0, _y));
		y.SetElement(0, _y, d.GetElement(0, _y));
		for (_x = 1; _x < n; _x++)
		{
			l.SetElement(_x - 1, _y, c.GetElement(_x - 1, _y) / m.GetElement(_x - 1, _y));
			m.SetElement(_x, _y, a.GetElement(_x, _y) - l.GetElement(_x - 1, _y) * b.GetElement(_x - 1, _y));
			y.SetElement(_x, _y, d.GetElement(_x, _y) - l.GetElement(_x - 1, _y) * y.GetElement(_x - 1, _y));
		}

		//3. Backward substitutions (U*x=y, for x)
		SetElement(n - 1, _y, y.GetElement(n - 1, _y) / m.GetElement(n - 1, _y));
		for (_x = (n - 2); _x >= 0; _x--)
		{
			SetElement(_x, _y, (y.GetElement(_x, _y) - b.GetElement(_x, _y) * GetElement(_x + 1, _y)) / m.GetElement(_x, _y));
		}
	}*/


	double *p_m, *p_a, *p_c, *p_b, *p_y, *p_d, *p_l;
	p_m = m.GetData();
	p_a = a.GetData();
	p_c = c.GetData();
	p_b = b.GetData();
	...
	memcpy(p_m, p_a, m_nNumColumns * sizeof(double));
	memcpy(p_y, p_d, m_nNumColumns * sizeof(double));
	...
	for (_y = 0; _y < m_nNumColumns; _y++)
	{
		...
		for (_x = 1; _x < n; _x++)
		{
			pos = _x * m_nNumColumns + _y;
			pos_1 = pos - m_nNumColumns;
			tmp = p_c[pos_1] / p_m[pos_1];
			p_l[pos_1] = tmp;
			p_m[pos] = p_a[pos] - tmp * (p_b[pos_1]);
			p_y[pos] = p_d[pos] - tmp * (p_y[pos_1]);
		}
		...
		for (_x = (n - 2); _x >= 0; _x--)
		{
			pos = _x * m_nNumColumns + _y;
			pos_1 = pos + m_nNumColumns;
			m_pData[pos] = (p_y[pos] - (p_b[pos]) * (m_pData[pos_1])) / (p_m[pos]);
		}
	}
}
...
//////////////////////////////////////////////////////////////////////
// 求任意实矩阵的特征值与特征向量（按特征值实部的升序排列，特征向量按行排列）
//
// 参数：
// 1. double dblU[] - 一维数组，长度为矩阵的阶数，返回时存放特征值的实部（升序排列）
// 2. double dblV[] - 一维数组，长度为矩阵的阶数，返回时存放特征值的虚部
// 3. CMatrix& mtxEigenVector - 返回时存放特征向量矩阵，其中第i行为与
//    数组dblU中第i个特征值对应的特征向量
// 4. double eps - 计算精度，默认值为0.000001
//
// 返回值：BOOL型，求解是否成功
//////////////////////////////////////////////////////////////////////
BOOL CMatrix::Eig(double dblU[], double dblV[], CMatrix& mtxEigenVector, double eps)
{
	CMatrix	tmp(*this) ;	
	tmp.MakeHberg();
	tmp.HBergEigenv(dblU,dblV,60,eps);
	int i,j;
	double t;
	for (i = 1;i < m_nNumColumns;i++)
	{
		for (j = 0; j < (m_nNumColumns - i);j++)
		{
			if (dblU[j] > dblU[j+1])
			{
				t = dblU[j];
	...
	CMatrix I;
	I.MakeUnitMatrix(m_nNumColumns);

	for (i = 0;i < m_nNumColumns;i++)
	{
		tmp = (*this) - (I * dblU[i]);
		tmp.InvertGaussJordan();
		CMatrix z(m_nNumColumns,1);
		z.Fill(1);
		CMatrix z1;
		CMatrix y;
		do 
		{
			...
			double m = y.Max_fabs();
			z = y / m;
		} while(z.Delta_fabs(z1) >= eps);
		for (j=0;j < m_nNumColumns;j++)
		{
			mtxEigenVector.SetElement(i,j,z.GetElement(j,0));
		}
	}
	mtxEigenVector.Gram_Schmidt();
	return TRUE;
}

void CMatrix::Fill(double value)
{
	if(m_pData)
	{
		for (int i = 0 ; i < m_nNumRows ; ++i)
...
double CMatrix::Delta_fabs(const CMatrix& other) const
{
	// 首先检查行列数是否相等
	ASSERT (m_nNumColumns == other.GetNumColumns() && m_nNumRows == other.GetNumRows());
	...
	for (int i = 0 ; i < m_nNumRows ; ++i)
	{
		for (int j = 0 ; j <  m_nNumColumns; ++j)
		{
			if (fabs(GetElement(i,j) - other.GetElement(i,j)) > x)
			{
				x = fabs(GetElement(i,j) - other.GetElement(i,j));
			}
		}
	}
	return x;
...
//标准正交化，假设向量以行的方式存放
void CMatrix::Gram_Schmidt()
{
/*	CMatrix a(GetRowVector(0));
	SetRowVector(0,a / sqrt(a.Dot(a)));
	CMatrix e;
	for (int n=1; n<m_nNumRows; ++n)
	{
		a = GetRowVector(n);
		CMatrix b(1,m_nNumColumns);
		for (int i=0;i < n;i++)
		{
			e = GetRowVector(i);
			b = b + (e * (a.Dot(e)));
		...
		a = a - b;
		SetRowVector(n,a / sqrt(a.Dot(a)));
	}*/

	for (int n=0; n<m_nNumRows; ++n)
	{
		CMatrix a(GetRowVector(n));
		SetRowVector(n,a / sqrt(a.Dot(a)));
	}
}

CMatrix CMatrix::sqrtm2D() const
{
	ASSERT(m_nNumRows == 2 && m_nNumColumns == 2);
	
	// 构造目标矩阵
	CMatrix	sqrtm(m_nNumRows, m_nNumColumns);

	double tr = GetElement(0,0) + GetElement(1,1);
	double delta = sqrt(GetElement(0,0) * GetElement(1,1) - GetElement(0,1) * GetElement(1,0));
	double factor = 1.0 / sqrt(tr + 2 * delta);
	sqrtm.SetElement(0, 0, factor * (delta + GetElement(0,0)));
	sqrtm.SetElement(0, 1, factor * (GetElement(0,1)));
	sqrtm.SetElement(1, 0, factor * (GetElement(1,0)));
	sqrtm.SetElement(1, 1, factor * (delta + GetElement(1,1)));
	
	return sqrtm;
}

CMatrix CMatrix::invert2D() const
{
	ASSERT(m_nNumRows == 2 && m_nNumColumns == 2);
	
	// 构造目标矩阵
	CMatrix	invert(m_nNumRows, m_nNumColumns);
	double a,b,c,d;
	a = GetElement(0,0);
	b = GetElement(0,1);
	c = GetElement(1,0);
	...
	invert.SetElement(0, 0, factor * d);
	invert.SetElement(0, 1, factor * (- b));
	invert.SetElement(1, 0, factor * (- c));
	invert.SetElement(1, 1, factor * a);

	return invert;
}

void CMatrix::invert_sqrtm_2D(CMatrix* inv_s) const
{
	ASSERT(m_nNumRows == inv_s->GetNumRows() && m_nNumRows == 2 && m_nNumColumns == inv_s->GetNumColumns() && m_nNumColumns == 2);
	double delta = sqrt(GetElement(0,0) * GetElement(1,1) - GetElement(0,1) * GetElement(1,0));
	double factor = 1.0 / sqrt(GetElement(0,0) + GetElement(1,1) + 2 * delta);
	double a,b,c,d;
	a = delta + GetElement(0,0);
	b = GetElement(0,1);
	c = GetElement(1,0);
	d = delta + GetElement(1,1);
	factor = 1.0 / (factor*(a * d - b * c));
	inv_s->SetElement(0, 0, factor * d);
	inv_s->SetElement(0, 1, factor * (- b));
	inv_s->SetElement(1, 0, factor * (- c));
	inv_s->SetElement(1, 1, factor * a);
}
