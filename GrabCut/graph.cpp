...
template <typename captype, typename tcaptype, typename flowtype>
	Graph<captype, tcaptype, flowtype>::Graph(const Graph& base_graph)
	:node_num(base_graph.node_num),
	nodeptr_block(NULL),
	error_function(base_graph.error_function)
{
	int node_num_max, edge_num_max, arc_num;
	node_num_max = base_graph.node_max - base_graph.nodes;
	edge_num_max = base_graph.arc_max - base_graph.arcs;
	arc_num = base_graph.arc_last - base_graph.arcs;

	nodes = (node*) malloc(node_num_max*sizeof(node));
	arcs = (arc*) malloc(edge_num_max*sizeof(arc));
	if (!nodes || !arcs) { if (error_function) (*error_function)("Not enough memory!"); exit(1); }

	memcpy(nodes,base_graph.nodes,node_num_max*sizeof(node));
	memcpy(arcs,base_graph.arcs,edge_num_max*sizeof(arc));

	node_last = nodes + base_graph.node_num;
	node_max = nodes + node_num_max;
	arc_last = arcs + arc_num;
	arc_max = arcs + edge_num_max;

	maxflow_iteration = base_graph.maxflow_iteration;
	flow = base_graph.flow;
}
