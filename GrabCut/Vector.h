...
public: 
  Vector(); 
  Vector(const unsigned int n, const InitValue init = DoInit);  
  ...
  inline Vector<T>& toLog();
  Vector<T> extract(const std::set<unsigned int>& indexes) const;
  inline T& operator[](const unsigned int& i); //i-th element 
  inline const T& operator[](const unsigned int& i) const; 
  inline void set(const unsigned int& i, const T& a); 
  inline const T& get(const unsigned int& i) const; //i-th element
  inline T* addr();
  inline void sub(const unsigned int& i, const T& a); 
  inline CString toString(const CString& sDelim = " ", const CString& sFormat = "%f") const;
  inline void toString(CString& outS, const CString& sDelim = " ", const CString& sFormat = "%f") const;
  inline void toFile(const CString& sPahtName) const;
	
  inline unsigned int size() const;
  inline void resize(const unsigned int n, const InitValue init = DoInit);
  inline void resize(const T& a, const unsigned int n);
	
  inline Vector<T>& operator=(const Vector<T>& rhs); //assignment 
  inline Vector<T>& operator=(const T& a); //assign a to every element 
  inline Vector<T>& operator+=(const Vector<T>& rhs);
  inline Vector<T>& operator-=(const Vector<T>& rhs);
  inline Vector<T>& operator*=(const Vector<T>& rhs);
  inline Vector<T>& operator/=(const Vector<T>& rhs);
  inline Vector<T>& divby(const Vector<T>& rhs);
  inline Vector<T>& operator^=(const Vector<T>& rhs);
  inline Vector<T>& operator+=(const T& a);
  inline Vector<T>& operator-=(const T& a);
  inline Vector<T>& operator*=(const T& a);
  inline Vector<T>& operator/=(const T& a);
  inline Vector<T>& divby(const T& a);
  inline Vector<T>& operator^=(const T& a);
...
template <typename T> 
Vector<T>::Vector(const unsigned int n, const InitValue init) 
  : v(new T[n]) 
{
	if (init == DoInit)
	{
...
template <typename T> 
void Vector<T>::resize(const unsigned int n, const InitValue init) 
{
	if (n == this->n)
	{
		if (init == DoInit)
...
template <typename T>
inline void Vector<T>::set(const unsigned int& i, const T& a)
{
	v[i] = a;
}

template <typename T>
inline void Vector<T>::sub(const unsigned int& i, const T& a)
{
	v[i] -= a;
}

template <typename T>
inline const T& Vector<T>::get(const unsigned int& i) const //subscripting 
{ 
  return v[i]; 
} 

template <typename T> 
inline T* Vector<T>::addr() 
{ 
  return v; 
}

template <typename T>
inline void Vector<T>::toString(CString& outS, const CString& sDelim, const CString& sFormat) const
{
	for (unsigned int i=0; i<n; ++i)
	{
		CString ss;
		ss.Format(sFormat, v[i]);
		outS += ss;
...
template <typename T> 
inline CString Vector<T>::toString(const CString& sDelim, const CString& sFormat) const
{ 
	CString s="";
...
template <typename T> 
inline void Vector<T>::toFile(const CString& sPahtName) const
{ 
	CString s = "";
	toString(s, " ", "%10.4f");
	CFile file;
...
template <typename T> 
inline void Vector<T>::set(const T* a) 
{ 
	set(a, n);
} 

template <typename T> 
inline Vector<T>& Vector<T>::toLog() 
{ 
	for (unsigned int i = 0; i < n; i++)
		v[i] = log(v[i]);
...
template <typename T> 
inline Vector<T>& Vector<T>::divby(const Vector<T>& rhs)
{
	if (this->size() != rhs.size())
		throw std::logic_error("divby: vectors have different sizes");
	for (unsigned int i = 0; i < n; i++)
		v[i] = rhs[i] / v[i];
...
template <typename T> 
inline Vector<T>& Vector<T>::divby(const T& a)
{
	for (unsigned int i = 0; i < n; i++)
		v[i] = a / v[i];
...
template <typename T>
inline T value_max(const Vector<T>& v)
{
  T value = v[0];
  for (unsigned int i = 1; i < v.size(); i++)
    value = __max(v[i], value);
	
  return value;
}

template <typename T>
inline T value_min(const Vector<T>& v)
{
  T value = v[0];
  for (unsigned int i = 1; i < v.size(); i++)
    value = __min(v[i], value);
...
// dst = src*a
template <typename T>
inline void mul(Vector<T>& dst, const Vector<T>& src, const T& a)
{
	unsigned int i, cnt;
	cnt = dst.size();
	if (cnt != src.size())
		throw std::logic_error("mul error: the vectors are not the same size");
	for (i = 0; i < cnt; i++)
		dst[i] = src[i] * a;
}
/**
...
{
  register unsigned int d = __min(n, m), i;
